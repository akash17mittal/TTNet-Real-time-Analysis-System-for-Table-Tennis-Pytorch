...
def create_target_ball(ball_position_xy, sigma, w, h, thresh_mask, device):
    ...
    if (w > ball_position_xy[0] > 0) and (h > ball_position_xy[1] > 0):
        ...
        target_ball_position[:w] = gaussian_1d(
            x_pos, ball_position_xy[0], sigma=sigma)
        # For y
        y_pos = torch.arange(0, h, device=device)
        target_ball_position[w:] = gaussian_1d(
            y_pos, ball_position_xy[1], sigma=sigma)
...
def get_events_infor(game_list, configs, dataset_type):
    ...
    for game_name in game_list:
        ball_annos_path = os.path.join(
            annos_dir, game_name, 'ball_markup.json')
        events_annos_path = os.path.join(
            annos_dir, game_name, 'events_markup.json')
        ...
        for event_frameidx, event_name in events_annos.items():
            ...
            for smooth_idx in smooth_frame_indices:
                ...
                if '{}'.format(last_f_idx) not in ball_annos.keys():
                    print(
                        'smooth_idx: {} - no ball position for the frame idx {}'.format(smooth_idx, last_f_idx))
                    continue
                ball_position_xy = ball_annos['{}'.format(last_f_idx)]
                ball_position_xy = np.array(
                    [ball_position_xy['x'], ball_position_xy['y']], dtype=np.int)
                ...
                if not os.path.isfile(seg_path):
                    print(
                        "smooth_idx: {} - The segmentation path {} is invalid".format(smooth_idx, seg_path))
                    continue
...
def train_val_data_separation(configs):
    ...
    events_infor, events_labels = get_events_infor(
    ...
    if configs.no_val:
        ...
        train_events_infor, val_events_infor, train_events_labels, val_events_labels = train_test_split(
            events_infor, events_labels, shuffle=True, test_size=configs.val_size, random_state=configs.seed, stratify=events_labels)
...
if __name__ == '__main__':
    ...
    train_events_infor, val_events_infor, train_events_labels, val_events_labels = train_val_data_separation(
        configs)
    print('Counter train_events_labels: {}'.format(Counter(train_events_labels)))
    if val_events_labels is not None:
        print(
            'Counter val_events_labels: {}'.format(
                Counter(val_events_labels)))
    event_name = 'net'
    event_class = configs.events_dict[event_name]
    configs.device = torch.device('cpu')
    ball_position_xy = np.array([100, 50])
    target_ball_position = create_target_ball(
        ball_position_xy,
        sigma=0.5,
        w=320,
        h=128,
    ...
    print(
        'target_ball_g_x: {}, target_ball_g_x: {}'.format(
            target_ball_g_x,
            target_ball_g_y))
