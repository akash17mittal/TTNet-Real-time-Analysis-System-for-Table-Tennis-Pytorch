    ...
    def __call__(self, imgs, ball_position_xy, seg_img):
        if random.random() <= self.p:
            for t in self.transforms:
                imgs, ball_position_xy, seg_img = t(
                    imgs, ball_position_xy, seg_img)
...
class Normalize():
    def __init__(
        ...
        self.p = p
        self.mean = np.repeat(
            np.array(mean).reshape(
                1,
                1,
                3),
            repeats=num_frames_sequence,
            axis=-1)
        self.std = np.repeat(
            np.array(std).reshape(
                1,
                1,
                3),
            repeats=num_frames_sequence,
    ...
    def __call__(self, imgs, ball_position_xy, seg_img):
        if random.random() <= self.p:
            ...
            ball_position_xy = np.array(
                [ball_position_xy[0] / w_ratio, ball_position_xy[1] / h_ratio])
...
class Random_Crop(object):
    ...
    def __call__(self, imgs, ball_position_xy, seg_img):
        ...
        if random.random() <= self.p:
            ...
            remain_percent = random.uniform(
                1. - self.max_reduction_percent, 1.)
            ...
            if (seg_img_h != h) or (seg_img_w != w):
                seg_img = cv2.resize(
                    seg_img, (w, h), interpolation=self.interpolation)
            # 2. Crop
            seg_img = seg_img[min_y:max_y, min_x:max_x, :]
            # 3. Resize to (128, 320, 3)
            seg_img = cv2.resize(
                seg_img, (seg_img_w, seg_img_h), interpolation=self.interpolation)
...
class Random_Rotate(object):
    ...
    def __call__(self, imgs, ball_position_xy, seg_img):
        if random.random() <= self.p:
            random_angle = random.uniform(-self.rotation_angle_limit,
            ...
            imgs = cv2.warpAffine(
                imgs, rotate_matrix, (w, h), flags=cv2.INTER_LINEAR)

            # Adjust ball position, apply the same rotate_matrix for the
            # sequential images
            ball_position_xy = rotate_matrix.dot(
                np.array([ball_position_xy[0], ball_position_xy[1], 1.]).T)

            # Rotate seg_img
            seg_h, seg_w, seg_c = seg_img.shape
            if (seg_h != h) or (seg_w != w):
                seg_center = (int(seg_w / 2), int(seg_h / 2))
                seg_rotate_matrix = cv2.getRotationMatrix2D(
                    seg_center, random_angle, 1.)
            else:
                seg_rotate_matrix = rotate_matrix
            seg_img = cv2.warpAffine(
                seg_img, seg_rotate_matrix, (seg_w, seg_h), flags=cv2.INTER_LINEAR)
