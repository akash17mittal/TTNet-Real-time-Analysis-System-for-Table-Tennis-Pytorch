...
class TTNet_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for img_path in img_path_list:
            ...
            resized_imgs.append(
                cv2.resize(
                    self.jpeg_reader.decode(
                        in_file.read(), 0), (self.w_input, self.h_input)))
            in_file.close()
        resized_imgs = np.dstack(resized_imgs)  # (128, 320, 27)
        # Adjust ball pos: full HD --> (320, 128)
        global_ball_pos_xy = self.__resize_ball_pos__(
            org_ball_pos_xy, self.w_resize_ratio, self.h_resize_ratio)
        ...
        org_ball_pos_xy = self.__resize_ball_pos__(
            global_ball_pos_xy,
            1. / self.w_resize_ratio,
            1. / self.h_resize_ratio)
        # If the ball position is outside of the resized image, set position to
        # -1, -1 --> No ball (just for safety)
        ...
        return resized_imgs, org_ball_pos_xy.astype(
            np.int), global_ball_pos_xy.astype(
...
if __name__ == '__main__':
    ...
    train_events_infor, val_events_infor, * \
        _ = train_val_data_separation(configs)
    ...
    if 1:
        ...
        origin_imgs = F.interpolate(torch.from_numpy(
            resized_imgs).unsqueeze(0).float(), (1080, 1920))
    ...
    out_images_dir = os.path.join(
        configs.results_dir, 'debug', 'ttnet_dataset')
    ...
    fig.suptitle(
        'Event: is bounce {}, is net: {}, ball_position_xy: (x= {}, y= {})'.format(
            target_event[0],
            target_event[1],
            org_ball_pos_xy[0],
            org_ball_pos_xy[1]),
    ...
    for i in range(configs.num_frames_sequence):
        ...
        if (i == (configs.num_frames_sequence - 1)):
            ...
            ball_img = cv2.circle(
                img, tuple(global_ball_pos_xy), radius=5, color=(
                    255, 0, 0), thickness=2)
