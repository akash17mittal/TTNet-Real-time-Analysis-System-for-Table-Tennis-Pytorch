...
class Multi_Task_Learning_Model(nn.Module):
    ...
    def __init__(
        ...
        self.event_loss_criterion = Events_Spotting_Loss(
            weights=weights_events, num_events=num_events)
    ...
    def forward(
        ...
        # Create target for events spotting and ball position (local and
        # global)
        ...
        for sample_idx in range(batch_size):
            target_ball_global[sample_idx] = create_target_ball(
                ...
                device=self.device)
        global_ball_loss = self.ball_loss_criterion(
            pred_ball_global, target_ball_global)
        total_loss = global_ball_loss / \
            (torch.exp(2 * self.log_vars[log_vars_idx])) + self.log_vars[log_vars_idx]
        ...
        if pred_ball_local is not None:
            ...
            for sample_idx in range(batch_size):
                target_ball_local[sample_idx] = create_target_ball(
                    ...
                    thresh_mask=self.thresh_ball_pos_mask,
                    device=self.device)
            local_ball_loss = self.ball_loss_criterion(
                pred_ball_local, target_ball_local)
            total_loss += local_ball_loss / \
                (torch.exp(2 * self.log_vars[log_vars_idx])) + self.log_vars[log_vars_idx]
        ...
        if pred_events is not None:
            ...
            total_loss += event_loss / \
                (2 * torch.exp(self.log_vars[log_vars_idx])) + self.log_vars[log_vars_idx]
    ...
    def run_demo(self, resize_batch_input):
        pred_ball_global, pred_ball_local, pred_events, pred_seg = self.model.run_demo(
            resize_batch_input)
